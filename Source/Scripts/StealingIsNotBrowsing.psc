...
Function TimePickPocket()
    ...
    if (UI.IsMenuOpen("ContainerMenu") && !owner.IsDead() && !owner.IsPlayerTeammate() && FindEffect())     
...
Event OnPlayerLoadGame()
    ...
    ShowNotif("actor value used is " + ActorValueMod)    
EndEvent

Event OnInit()
    RegisterForSingleUpdate(5) ;to prevent double OnInit    
EndEvent
...
Event OnMenuOpen(String MenuName)    
    ...
    ;--------------set cap for timer-------------------------
        ...
        StartMenuModeTimer(self.GetOwningQuest(), Timer, 15268)
    else
        ShowNotif("no pickpocketing")
    endif
endevent
...
Event OnMenuClose(String MenuName)
    if MenuName == "ContainerMenu"
        CancelMenuModeTimer(self.GetOwningQuest(), 15268)
    endif
Endevent
